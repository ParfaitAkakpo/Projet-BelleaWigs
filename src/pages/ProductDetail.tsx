...
export default function ProductDetail() {
 ...
 return /\.(mp4|webm|ogg)(\?.*)?$/i.test(url);
  ...
  const medias = useMemo(() => carouselItems.map((it) => it.imageUrl), [carouselItems]);
...
const handleQtyInputChange = (raw: string) => {
  ...
  // autoriser le champ vide pendant que l’utilisateur tape
...
const handleQtyBlur = () => {
  ...
  // Related products: same category first, then same family, then close price
  const relatedProducts = useMemo(() => {
    ...
    return scored.slice(0, 8); // ⬅️ tu peux mettre 12 si tu veux
  }, [products, product]);
      ...
      >
        ...
        {isSelected && (
          <Check className="absolute inset-0 m-auto h-5 w-5 text-white drop-shadow-md" />
            ...
            {/* Details list (si tu ajoutes details en DB, sinon ça ne s’affiche pas) */}
